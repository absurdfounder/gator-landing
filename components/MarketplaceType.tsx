...
const MarketplaceCard: React.FC<MarketplaceCardProps> = ({ title, description, index, tag }) => (
  ...
  <Link
    href={`/integration`}
    className="group bg-white p-4 sm:p-6 rounded-lg border shadow-md h-full w-full transition-all duration-200 hover:shadow-lg hover:border-emerald-300 block relative focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2" // Added group, block, relative, hover/focus effects
  ...
  >
    ...
    <div
      className="flex items-center text-emerald-500 font-medium transition-opacity duration-300 ease-in-out opacity-0 group-hover:opacity-100 absolute bottom-4 right-4 sm:bottom-6 sm:right-6" // Show on hover, positioned bottom-right
